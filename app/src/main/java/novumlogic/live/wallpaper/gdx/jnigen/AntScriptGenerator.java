...
import novumlogic.live.wallpaper.gdx.jnigen.BuildTarget.TargetOs;
import novumlogic.live.wallpaper.gdx.jnigen.FileDescriptor.FileType;
...
public class AntScriptGenerator {
	...
	public void generate (BuildConfig config, BuildTarget... targets) {
		...
		if (config.jniDir.child("memcpy_wrap.c").exists() == false) {
			new FileDescriptor("novumlogic.live.wallpaper.gdx.jnigen/resources/scripts/memcpy_wrap.c", FileType.Classpath).copyTo(config.jniDir
				.child("memcpy_wrap.c"));
		}
		...
		String template = new FileDescriptor("novumlogic.live.wallpaper.gdx.jnigen/resources/scripts/build.xml.template", FileType.Classpath)
			.readString();
	...
	private void copyJniHeaders (String jniDir) {
		final String pack = "novumlogic.live.wallpaper.gdx.jnigen/resources/headers";
	...
	private String generateBuildTargetTemplate (BuildConfig config, BuildTarget target) {
		...
		if (target.os == TargetOs.Android) {
			...
			String template = new FileDescriptor("novumlogic.live.wallpaper.gdx.jnigen/resources/scripts/build-android.xml.template",
		...
		if (target.os == TargetOs.IOS) {
			template = new FileDescriptor("novumlogic.live.wallpaper.gdx.jnigen/resources/scripts/build-ios.xml.template", FileType.Classpath)
				.readString();
		} else {
			template = new FileDescriptor("novumlogic.live.wallpaper.gdx.jnigen/resources/scripts/build-target.xml.template", FileType.Classpath)
				.readString();
		}
